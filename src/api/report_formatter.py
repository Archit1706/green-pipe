...
from src.api.analytics_schemas import LeaderboardEntry
...
def _sanitize_md(text: str, max_len: int = 200) -> str:
    """Sanitize user-controlled text for safe markdown embedding.

    Strips backticks, angle brackets, and pipe characters that could
    break table formatting or inject HTML/markdown.
    """
    safe = text[:max_len].replace("`", "").replace("<", "&lt;").replace(">", "&gt;").replace("|", "/")
    return safe
...
def format_mr_comment(report: PipelineAnalysisReport) -> str:
    ...
    lines += [
        ...
        "",
        f"*{pipeline_ref} · "
        f"`{_sanitize_md(report.pipeline_ref or 'unknown ref')}` @ "
        f"`{_sanitize_md((report.pipeline_sha or '')[:8] or 'unknown sha')}` · "
    ...
    for jr in report.job_reports:
        lines.append(
            f"| `{_sanitize_md(jr.job_name or 'unknown')}` "
            f"| {_sanitize_md(jr.stage or '—')} "
            f"| {jr.duration_seconds:.0f}s "
            f"| `{_sanitize_md(jr.runner_type or 'unknown')}` "
...
def format_leaderboard_comment(
    entries: list[LeaderboardEntry], period: str = "all-time"
) -> str:
    """
    Return a GitLab markdown comment for a contributor carbon leaderboard.

    Posted in response to ``@greenpipe leaderboard``.
    """
    lines: list[str] = [
        f"## 🏆 GreenPipe Carbon Leaderboard — Top {len(entries)} Green Contributors ({period.title()})",
        "",
    ]

    if not entries:
        lines += [
            "> ⚠️ No contributor data available. "
            "Analyse pipelines to populate the leaderboard.",
            "",
        ]
        return "\n".join(lines)
    ...
    for e in entries:
        rank_icon = _RANK_ICONS.get(e.rank, f"{e.rank}.")
        deferred_str = f"{e.deferred_count} ({e.deferred_percent:.0f}%)"
        lines.append(
            f"| {rank_icon} {e.rank} "
            f"| {_sanitize_md(e.author_name)} "
            f"| {e.pipeline_count} runs "
            f"| {e.avg_sci_score:.1f} gCO₂e "
            f"| {deferred_str} "
            f"| {e.co2e_saved_gco2e:.1f} g |"
        )

    lines += [
        "",
        "> 💡 Lower SCI = greener pipelines. "
        "Defer non-urgent pipelines to improve your ranking!",
        "",
        "*Generated by [🌱 GreenPipe](https://github.com/greenpipe) "
        "— gamified carbon tracking for CI/CD*",
    ]
